...
def make_plots_meg_qc(dataset_path: str, n_jobs: int = 1, derivatives_base: Optional[str] = None):
    ...
    # All derivative lookups must point to the resolved output root so that
    # externally redirected derivatives (``derivatives_base``) are found. Keep
    # the dataset scoped to ``output_root`` for the entire plotting workflow to
    # cover every query and the joblib workers.
    with temporary_dataset_base(dataset, output_root):
        # --------------------------------------------------------------------------------
