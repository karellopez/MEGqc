...
def get_derivatives_base_path(dataset_path: str, override_path: Optional[str]) -> str:
    ...
    When ``override_path`` is provided, derivatives are placed in a folder
    named after the BIDS dataset inside ``override_path``. This keeps
    derivatives grouped per dataset even when written outside the original
    BIDS root.
    ...
    if override_path:
        derivatives_base = os.path.join(os.path.abspath(override_path), dataset_name)
    else:
...
def make_derivative_meg_qc(
    ...
    for dataset_path in ds_paths:
        ...
        delete_temp_folder(derivatives_base)
